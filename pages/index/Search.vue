...
import type {SuggestItem, TrendingItem} from "@/api/types/serach.ts";
...
defaultSearch.value = (await getDefaultSearch()).show_name

const searchFocus = async () => {
  trendingList.value = await getTrendingList()
  isClick.value = true
  addEvent()
}
